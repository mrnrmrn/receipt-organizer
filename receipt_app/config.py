...
STRUCTURED_EXTRACTION_GUIDE_TEXT = (
    "비목/세목 가이드:\n"
    "- 자기관리 / 의료/치료: 치과치료, 물리치료 등 일반 의료 목적에 사용한 비용 청구 가능 / 여러 개월에 걸쳐 사용 가능한 회원권(예: 헬스장 1년권)은 월별로 나누어 신청\n"
    "- 자기관리 / 헤어\n"
    "- 자기관리 / 운동\n"
    "- 식비 / 음식점: 일반 음식점만 청구 가능 / 주류는 청구 불가, 주류 금액은 제외 / 다른 팀원과 함께 이용한 금액은 N할로 분할 청구\n"
    "- 식비 / 배달\n"
    "- 통신비 / 통신비: 개인 및 업무 장비에 사용한 통신요금 포함\n"
    "- 교통비 / 대중교통\n"
    "- 교통비 / 택시: 택시는 동승 인원이 있으면 개인별 분할 청구"
)

ALLOWED_CATEGORY_SUBCATEGORY_PAIRS = (
    ("자기관리", "의료/치료"),
    ("자기관리", "헤어"),
    ("자기관리", "운동"),
    ("식비", "음식점"),
...
class AppConfig:
    ...
    structured_extraction_guide_text: str = STRUCTURED_EXTRACTION_GUIDE_TEXT
    allowed_category_subcategory_pairs: tuple[tuple[str, str], ...] = (
        ALLOWED_CATEGORY_SUBCATEGORY_PAIRS
    )
